...
/// process 64 bytes
void SHA__256::processBlock(const void* data)
{
    // get last hash
    uint32_t a = m_hash[0];
    uint32_t b = m_hash[1];
...
void SHA__256::add(const void* data, size_t numBytes)
{
...
void SHA__256::getHash(unsigned char buffer[SHA__256::HashBytes])
{
...
/// compute SHA__256 of a memory block
std::string SHA__256::operator()(const void* data, size_t numBytes)
{
    reset();
    add(data, numBytes);
    return getHash();
...
/// compute SHA__256 of a string, excluding final zero
std::string SHA__256::operator()(const std::string& text)
{
    reset();
